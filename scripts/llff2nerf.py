...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--downscale", type=float, default=1, help="image size down scale, e.g., 4"
    )
    parser.add_argument(
        "--hold", type=int, default=8, help="hold out for validation every $ images"
    )
    parser.add_argument(
        "--train_num", type=int, default=-1, help="sparseview train image num"
    )
    ...
    print(
        f"[INFO] loaded {len(images)} images, {N} poses_bounds as {poses_bounds.shape}"
    )
    ...
    write_json("transforms_val.json", frames_test[::5])
