...
class RaySamplerSingleImage(object):
    def __init__(self, data, device, resize_factor=1, render_stride=1, inds=None):
        super().__init__()
        ...
        self.rays_o, self.rays_d = self.get_rays_single_image(
            self.H, self.W, self.intrinsics, self.c2w_mat, inds
        )
        if self.rgb is not None:
    ...
    def get_rays_single_image(self, H, W, intrinsics, c2w, inds=None):
        """
        ...
        u = torch.from_numpy(
            u.reshape(-1).astype(dtype=np.float32)
        )  # + 0.5    # add half pixel
        v = torch.from_numpy(v.reshape(-1).astype(dtype=np.float32))  # + 0.5
        if inds is not None:
            u = u.to(inds.device)
            v = v.to(inds.device)
    ...
    def get_all(self):
        ret = {
            ...
            "src_cameras": self.src_cameras.cuda()
