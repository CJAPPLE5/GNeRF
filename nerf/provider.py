...
def slerp_pose(pose0, pose1, ratio):
    rots = Rotation.from_matrix(np.stack([pose0[:3, :3], pose1[:3, :3]]))
    slerp = Slerp([0, 1], rots)

    ratio = 0.5
    pose_s = np.eye(4, dtype=np.float32)
    pose_s[:3, :3] = slerp(ratio).as_matrix()
    pose_s[:3, 3] = (1 - ratio) * pose0[:3, 3] + ratio * pose1[:3, 3]
    return pose_s
...
class NeRFDataset:
    def __init__(self, opt, device, type="train", downscale=1, n_test=10):
        ...
        self.use_ibr = opt.use_ibr  # if use_ibr, load extra info
        ...
        if self.mode == "colmap" and type == "test":
            ...
            for f in tqdm.tqdm(frames, desc=f"Loading {type} data"):
                ...
                if "bounds" in f:
                    bounds = np.array(f["bounds"], dtype=np.float32)
        ...
        if len(self.bounds) > 0:
            self.bounds = torch.from_numpy(np.stack(self.bounds, axis=0))
        ...
        self.intrinsic_m = torch.eye(4)
        self.intrinsic_m[0, 0] = fl_x
        self.intrinsic_m[1, 1] = fl_y
        self.intrinsic_m[0, 2] = cx
        self.intrinsic_m[1, 2] = cy
        self.hw = torch.tensor([self.H, self.W])
        if self.preload:
            self.hw = self.hw.to(self.device)
            self.intrinsic_m = self.intrinsic_m.to(self.device)
        if self.use_ibr:
            self.cameras = []
            ...
            results["camera"] = (
                torch.concat(
                    [
                        self.hw,
                        self.intrinsic_m.reshape(-1),
                        unseen_pose.reshape(-1),
                    ]
                )
                .unsqueeze(0)
                .to(self.device)
            )
            other_index = [k for k in range(self.poses.shape[0]) if k != index]
            results["rgb_path"] = "wdnmd_path"
            results["src_cameras"] = (
                self.cameras[other_index].to(self.device).unsqueeze(0)
            )
            # results["depth_range"] = self.bounds[index].to(self.device)
            results["depth_range"] = torch.tensor([[0.1, 6.0]]).to(self.device)
        return results
